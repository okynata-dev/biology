...
- **Watch `_headers` for duplicate path blocks — CF Pages merges them PER-HEADER, last-wins on conflict.** Verified live 2026-05-29: `/lab` is declared in two blocks (a security block with CSP+XFO, a cache block with Cache-Control) and the response carries ALL THREE headers — so CF does merge across blocks, header-by-header, not "last whole block wins" as an earlier version of this note claimed. The catch: when the SAME header name appears in two matching blocks, the later one wins. The `/preview` hero breakage on 2026-05-28 was exactly that — a second `/preview` block (Cache-Control) didn't carry the `frame-ancestors *` override, and because the wildcard `/*` block now sets `frame-ancestors 'none'` for everything, `/preview` needs its CSP stated in ONE consolidated block (it now is). Rule of thumb: security-critical headers (CSP, X-Frame-Options) live in the wildcard `/*` as the safe default; only override in a per-path block when you must, and keep that path's full header set in a single block. The benign duplicates (`/lab`, `/`, `/make`, etc.) are fine because the wildcard carries their security headers regardless.
